...
<div class="container">
    <div id="log-in">
        <form class="login-form" action="{{ route('frontend.auth.password.send_mail') }}" method="post">
            {{ csrf_field() }}
            @if(!empty($token))
            ...
            <div class="content">
                ...
                @if(session('status') == trans('passwords.token') || session('status') == trans('passwords.user'))
                    <div class="errors">
                        {{ session('status') }}
                    </div>
                @else
                    <div class="success" style="color: #0F9E5E">
                        {{ session('status') }}
                    </div>
                @endif
