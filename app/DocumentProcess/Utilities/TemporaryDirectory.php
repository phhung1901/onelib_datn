...
class TemporaryDirectory
{
    ...
    protected bool $autoDestroyed = true;
    ...
    public function tmpPath($suffix = '', $prefix = 'tmp', $ext_dot = '.')
    {
        $suffix = $suffix ? $ext_dot . $suffix : "";
        $fileName = $prefix . time() . \Str::random(4) . $suffix;
    ...
    public function path(string $pathOrFilename = ''): string
    {
        ...
        if (!file_exists($directoryPath)) {
            ...
            if (!is_dir($directoryPath)) {
                throw new \Exception(sprintf('Impossible to create the root directory "%s".', $directoryPath));
            }
        }
        return $path;
    }
    ...
    public function empty(): self
    {
        ...
        if (!is_dir($this->getFullPath())) {
            throw new \Exception(sprintf('Impossible to create the root directory "%s".', $this->getFullPath()));
        }
        return $this;
    }
