<?php

return [
    [
        'title' => 'Supermarkets (open) at 8 o’clock and',
        'source_url' => [
            'href' => 'https://web.ung.edu/media/Chemistry2/Chemistry-LR.pdf',
            'ses' => ['google'],
        ],
        'url' => base_path('files_for_faker/1.pdf'),
        'keywords' => 'che, chemistry, chemical',
        'topics' => 9,
    ],
    [
        'title' => 'Circulating levels of pentraxin-3 (PTX3) in patients with liver cirrhosis',
        'source_url' => 'https://web.ung.edu/media/Chemistry2/Chemistry-LR.pdf',
        'url' => base_path('files_for_faker/2.pdf'),
        'keywords' => 'Liver cirrhosis, Inflammation, Biomarkers, PTX3 protein, Prognosis, Acute-On-Chronic Liver Failure, Mortality, Business',
        'topics' => 10,
    ],
    [
        'title' => 'Neumonía por Pasteurella multocida en un adolescente. Reporte de un caso',
        'source_url' => 'https://web.ung.edu/media/Chemistry2/Chemistry-LR.pdf',
        'url' => base_path('files_for_faker/3.pdf'),
        'keywords' => 'Pasteurella multocida, neumonía, infección respiratoria, Education'
    ],
    [
        'title' => '被説明変数のとりうる値に制約のあるモデル',
        'source_url' => 'https://web.ung.edu/media/Chemistry2/Chemistry-LR.pdf',
        'url' => base_path('files_for_faker/12.pdf'),
        'keywords' => 'モデル,選択'
    ],
    [
        'title' => 'CV : HỌC VIỆN CÔNG NGHỆ BƯU CHÍNH VIỄN THÔNG',
        'source_url' => 'https://web.ung.edu/media/Chemistry2/Chemistry-LR.pdf',
        'url' => base_path('files_for_faker/5.pdf'),
        'keywords' => 'CV, Jobs, Career'
    ],
//    [
//        'title' => 'Ỳ THI CHỌN HỌC SINH GIỎI CẤP TỈNH LỚP 12 THPT NĂM HỌC 2012-2013',
//        'source_url' => 'https://web.ung.edu/media/Chemistry2/Chemistry-LR.pdf',
//        'url' => base_path('files_for_faker/6.pdf'),
//        'keywords' => 'Test, THPT, Thi'
//    ],
    [
        'title' => 'The Role of Net1 Phosphorylation in Regulating CDC14 Release During Mitotic Exit',
        'source_url' => 'https://kinhnghiemdayhoc.net/wp-content/uploads/2019/08/Giao-an-Hoat-dong-trai-nghiem-sang-tao-Lop-5.doc',
        'url' => 'https://kinhnghiemdayhoc.net/wp-content/uploads/2019/08/Giao-an-Hoat-dong-trai-nghiem-sang-tao-Lop-5.doc',
        'keywords' => 'Cooperative Phase Transitions, Dimyristoyl, Differential Scanning Calorimetry, DSC',
        'topics' => 9,
    ],
//    [
//        'title' => 'Micromechanics-inspired three-dimensional constitutive model for the thermomechanical response of shape-memory alloys',
//        'source_url' => 'https://thesis.library.caltech.edu/1725/1/Thesis_AmirSadjadpour.pdf',
//        'url' => 'https://thesis.library.caltech.edu/1725/1/Thesis_AmirSadjadpour.pdf',
//        'keywords' => 'Constitutive Modeling, Martensitic Phase Transformation, NiTi, Pure Iron, Shape Memory Alloys, SMA',
//        'topics' => 9,
//    ],
//    [
//        'title' => 'Differential Scanning Calorimetry of Bilayer Membrane Phase Transitions',
//        'source_url' => 'http://www.colby.edu/chemistry/PChem/lab/DSCMembrane.pdf',
//        'url' => 'http://www.colby.edu/chemistry/PChem/lab/DSCMembrane.pdf',
//        'keywords' => 'Cooperative Phase Transitions, Dimyristoyl, Differential Scanning Calorimetry, DSC',
//        'topics' => 9,
//    ],
//    [
//        'title' => 'Introductory Physics 1',
//        'source_url' => 'https://webhome.phy.duke.edu/~rgb/Class/intro_physics_1/intro_physics_1.pdf',
//        'url' => 'https://webhome.phy.duke.edu/~rgb/Class/intro_physics_1/intro_physics_1.pdf',
//        'keywords' => 'intro, physics'
//    ],
//    [
//        'title' => 'Chemistry LR',
//        'source_url' => 'https://web.ung.edu/media/Chemistry2/Chemistry-LR.pdf',
//        'url' => 'https://web.ung.edu/media/Chemistry2/Chemistry-LR.pdf',
//        'keywords' => 'che, chemistry, chemical'
//    ],
    [
        'title' => 'CHUYÊN ĐỀ 1 : GIÁ TRỊ LƯỢNG GIÁC CỦA MỘT GÓC BẤT KỲ',
        'source_url' => 'https://web.ung.edu/media/Chemistry2/Chemistry-LRR.pdf',
        'url' => base_path('files_for_faker/8.doc'),
        'keywords' => 'TOÁN, LƯỢNG GIÁC'
    ],
    [
        'title' => 'ĐOÀN THƯỢNG-HẢI DƯƠNG LẦN 2 NĂM 2019',
        'source_url' => 'https://web.ung.edu/media/Chemistry2/Chemistry-LRRD.pdf',
        'url' => base_path('files_for_faker/7.docx'),
        'keywords' => 'TOÁN, LƯỢNG GIÁC'
    ],
//    [
//        'title' => 'C-Terminal Flap Endonuclease (rad27) Mutations: Lethal Interactions With a DNA Ligase I Mutation (cdc9-p) and Suppression by Proliferating Cell Nuclear Antigen (POL30) in Saccharomyces cerevisiae',
//        'source_url' => 'https://www.genetics.org/content/suppl/2009/07/13/genetics.109.103937.DC1',
//        'url' => base_path('files_for_faker/10.pdf'),
//        'keywords' => 'C-Terminal Flap Endonuclease'
//    ],
//    [
//        'title' => 'MÉXICO Y LA INVERSIÓN EXTRANJERA: NUESTRA SITUACIÓN DENTRO DEL CONTEXTO INTERNACINAL',
//        'source_url' => 'https://www.genetics.org/content/suppl/2009/07/13/genetics.109.103937.DC2',
//        'url' => base_path('files_for_faker/11.pdf'),
//        'keywords' => 'C-Terminal Flap Endonuclease'
//    ],
//    [
//        'title' => 'A Similar document 1',
//        'source_url' => 'https://www.genetics.org/content/suppl/2009/07/13/genetics.109.103937.DC3',
//        'url' => base_path('files_for_faker/en_dup_1.pdf'),
//        'keywords' => 'C-Terminal Flap Endonuclease'
//    ],
//    [
//        'title' => 'A Similar document 2',
//        'source_url' => 'https://www.genetics.org/content/suppl/2009/07/13/genetics.109.103937.DC4',
//        'url' => base_path('files_for_faker/en_dup_2.pdf'),
//        'keywords' => 'C-Terminal Flap Endonuclease'
//    ],
];
